...
class Graph:
    __A = ['Ed', 'Du', 'Is', 'Br', 'Jp'] # stores the city name
    __n = 0 # number of vertices
    __g = [[0 for x in range(5)] for y in range(5)] # for the matrix
    ...
    def cycle_det(self, start_vertex, visited, pred, lc, local_pos, pred_local):
        ...
        if label_count == 1: # mark the first vertex
...
def menu():
    ...
    print("      3) Display the current graph")
